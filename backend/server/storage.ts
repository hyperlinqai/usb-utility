...
class DatabaseStorage implements IStorage {
    ...
    if (allSystemsWithMacId.length > 1) {
      ...
      if (exactMatch) {
        ...
        const updates: any = {
          ...
          // Ensure clientStatus is set to 1 (online) on every heartbeat
          clientStatus: 1,
        ...
        newSystem = await this.createSystem({
          ...
          machineOn: 1, // Online
          // Ensure clientStatus is set to 1 (online) for new systems
          clientStatus: 1,
          clientStatusUpdatedAt: now,
          lastConnected: now,
          lastUpdated: now, // Server timestamp for status check
  ...
  async resetExpiredClientStatus(): Promise<number> {
    try {
      ...
      // This is the strict offline threshold requested by the user
      const thresholdTime = new Date(now.getTime() - 60 * 1000);

      // Find all systems with client_status = 1 (online) and last_connected older than threshold
      // After threshold, reset to 0 (offline)
      const systemsToReset = await db
        ...
        .where(
          and(
            ...
            sql`${clientMaster.lastConnected} IS NOT NULL`,
            sql`${clientMaster.lastConnected} <= ${thresholdTime}`
          )
        );
      ...
      if (affectedRows > 0) {
        console.log(`[HEARTBEAT MONITOR] Reset ${affectedRows} system(s) from online to offline (no heartbeat for >60s)`);
      }
...
export function startClientStatusResetJob(): void {
  ...
  // Then run every 15 seconds to check frequently
  ...
  console.log('[HEARTBEAT MONITOR] Started - will check for expired heartbeats every 15 seconds');
}
...
export function startSetAllMachinesOfflineJob(): void {
  // DEPRECATED: This job is removed as it causes issues with persistent connections.
  // DEPRECATED: This job has been removed.
  console.log('[SET ALL MACHINES OFFLINE JOB] Disabled - Logic removed');
}
